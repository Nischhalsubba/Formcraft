...
(() => {
  ...
  function deviceCard(device, snapshot) {
    ...
    return `<article class="hrms-device-card">
      <header><span class="hrms-device-state ${live ? 'is-online' : 'is-offline'}"></span><div><strong>${esc(device.name)}</strong><small>${esc(device.ip_address)}:${esc(device.port)} ${device.force_udp ? 'UDP' : 'TCP'}</small></div>${status(device.active ? (live ? 'online' : 'offline') : 'inactive')}</header>
  ...
  function renderRawPunches() {
    ...
    let body;
    if (!snapshot) body = empty('Loading raw punches', 'The reader event log is loaded separately from the existing Attendance records.');
    else if (snapshot.schemaMissing) body = empty('Device bridge schema is not installed', 'Existing Attendance continues to work. Apply the additive bridge migration to use raw biometric logs.');
