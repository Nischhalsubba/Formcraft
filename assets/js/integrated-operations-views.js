...
(() => {
  ...
  const taskOptions = (excludeId = '', projectId = '') => [['', 'None']].concat(
    state.tasks
      .filter(task => task.id !== excludeId && (!projectId || task.projectId === projectId))
      .map(task => [task.id, `${task.key} · ${task.title}`])
  );
  const section = (title, copy, content) => `<fieldset class="bright-form-section">
    <legend>${escapeHtml(title)}</legend>
    <p class="bright-form-section-copy">${escapeHtml(copy)}</p>
    <div class="field-grid">${content}</div>
  </fieldset>`;

  function returnToVisibleRecord() {
    closeModal();
    renderShell();
  }

  openProjectForm = function openOperationsProjectForm(project = null) {
    ensure();
    ...
    const data = existing ? project : {
      name: '',
      code: codeFromName('', state.projects.length),
      client: '',
    ...
    const fields = `<div class="bright-form-sections">
      ...
      ${section('Schedule and delivery', 'Automatic progress uses completed task estimates.', `
        ...
        ${field('Manual progress', 'progress', data.progress, { type: 'number', min: 0, max: 100, step: 1, hint: 'Used only when progress calculation is manual.' })}
      `)}
      ...
      ${section('Project brief', 'Keep scope, constraints, and success criteria in one source of truth.', `
        ${field('Description', 'description', data.description, { textarea: true, span: true, required: true, maxlength: 4000, placeholder: 'Describe the outcome, scope, constraints, and success criteria.' })}
      `)}
    </div>`;

    openFormModal(
      existing ? 'Edit project' : 'Create project',
      'Create one connected record for tasks, schedule, time, files, and billing.',
      fields,
      async form => {
        ...
        returnToVisibleRecord();
        toast(existing ? 'Project updated.' : 'Project created.');
        if (!existing) requestAnimationFrame(() => openRecord('project', saved.id, { replace: true }));
      }
    );
  };

  openTaskForm = function openOperationsTaskForm(task = null) {
    ensure();
    if (!canEdit()) return toast('You have read-only access to this workspace.', 'warning');
    ...
    const data = existing ? task : {
      title: '',
      projectId: preset.projectId || state.projects[0].id,
      issueType: preset.issueType || 'task',
      parentTaskId: preset.parentTaskId || '',
      assigneeId: preset.assigneeId || currentUserId(),
      reporterId: preset.reporterId || currentUserId(),
      priority: preset.priority || 'medium',
      status: preset.status || 'todo',
      startDate: preset.startDate || dateKey(today()),
      dueDate: preset.dueDate || dateKey(addDays(7)),
      estimateHours: preset.estimateHours || 0,
      storyPoints: preset.storyPoints || 0,
      billable: Boolean(preset.billable),
      labels: arr(preset.labels),
    ...
    const fields = `<div class="bright-form-sections">
      ...
      ${section('Delivery context', 'The task record is the source of truth for implementation and acceptance.', `
        ${field('Labels', 'labels', arr(data.labels).join(', '), { span: true, maxlength: 300, hint: 'Separate labels with commas.' })}
        ${field('Description', 'description', data.description, { textarea: true, span: true, maxlength: 4000, placeholder: 'Describe the work, constraints, links, and implementation context.' })}
        ${field('Acceptance criteria', 'acceptanceCriteria', data.acceptanceCriteria, { textarea: true, span: true, maxlength: 4000, placeholder: 'State the observable conditions required for completion.' })}
      `)}
    </div>`;

    openFormModal(
      existing ? `Edit ${data.key}` : 'Create task',
      'Create a traceable work item linked to delivery, time, files, and billing.',
      fields,
      async form => {
        const values = formValues(form);
        ...
        if (existing && !canSetParent(task.id, values.parentTaskId)) throw new Error('That parent selection would create a circular task hierarchy.');
        ...
        requestAnimationFrame(() => openRecord(returnType, returnId, { replace: true }));
      }
    );

    requestAnimationFrame(() => {
      const form = modal.querySelector('[data-modal-form], form');
      if (!form?.elements.projectId || !form.elements.parentTaskId) return;
      form.elements.projectId.addEventListener('change', () => {
        const selected = form.elements.parentTaskId.value;
        form.elements.parentTaskId.innerHTML = taskOptions(existing ? task.id : '', form.elements.projectId.value)
  ...
  const taskDepth = (task, seen = new Set()) => {
    ...
    if (!parent) return 0;
    seen.add(task.id);
    return 1 + taskDepth(parent, seen);
  };
  const sortTasks = tasks => [...tasks].sort((a, b) => STATUSES.indexOf(a.status) - STATUSES.indexOf(b.status) || PRIORITIES.indexOf(b.priority) - PRIORITIES.indexOf(a.priority) || String(a.dueDate || '').localeCompare(String(b.dueDate || '')) || String(a.key || '').localeCompare(String(b.key || '')));
  ...
  function taskTable(tasks, includeProject = false) {
    ...
    return `<div class="ops-task-table-wrap"><table class="ops-task-table"><thead><tr><th><span class="sr-only">Done</span></th><th>Key</th><th>Summary</th>${includeProject ? '<th>Project</th>' : ''}<th>Type</th><th>Status</th><th>Priority</th><th>Assignee</th><th>Estimate</th><th>Spent</th><th>Due</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody>${tasks.map(task => {
      ...
      return `<tr class="${task.status === 'done' ? 'is-complete' : ''}"><td><input class="email-check" type="checkbox" data-toggle-task="${escapeHtml(task.id)}" ${task.status === 'done' ? 'checked' : ''} ${canEdit() ? '' : 'disabled'} aria-label="Mark ${escapeHtml(task.title)} as ${task.status === 'done' ? 'open' : 'complete'}"></td><td><button class="ops-key-button" type="button" data-ops-open-task="${escapeHtml(task.id)}">${escapeHtml(task.key)}</button></td><td><button class="ops-summary-button" type="button" data-ops-open-task="${escapeHtml(task.id)}" style="--task-depth:${taskDepth(task)}"><strong>${escapeHtml(task.title)}</strong><small>${escapeHtml([childLabel, task.labels.slice(0, 2).join(' · ')].filter(Boolean).join(' · ') || 'No labels')}</small></button></td>${includeProject ? `<td><button class="ops-link-button" type="button" data-view-project="${escapeHtml(project?.id || '')}">${escapeHtml(project?.name || 'Unassigned')}</button></td>` : ''}<td>${typeBadge(task.issueType)}</td><td>${statusBadge(task.status)}</td><td>${priorityBadge(task.priority)}</td><td>${escapeHtml(memberName(task.assigneeId))}</td><td>${task.estimateHours ? `${task.estimateHours}h` : '—'}</td><td>${taskHours(task.id) ? `${taskHours(task.id)}h` : '—'}</td><td>${escapeHtml(dual(task.dueDate))}</td><td>${overflowMenu([{ label: 'Open task', icon: 'eye', action: 'ops-open-task', id: task.id }, ...(canEdit() ? [{ label: 'Edit task', icon: 'edit', action: 'edit-task', id: task.id }, { label: 'Delete task', icon: 'trash', action: 'delete-task', id: task.id, danger: true }] : [])], `Actions for ${task.key}`)}</td></tr>`;
    }).join('')}</tbody></table></div>`;
  }

  const taskBoard = tasks => `<div class="ops-task-board">${STATUSES.map(status => {
    const items = tasks.filter(task => task.status === status);
    return `<section class="ops-task-column" data-ops-drop-status="${status}"><header>${statusBadge(status)}<strong>${items.length}</strong></header><div>${items.length ? items.map(task => `<button class="ops-task-card" type="button" data-ops-open-task="${escapeHtml(task.id)}" data-ops-drag-task="${escapeHtml(task.id)}" draggable="${canEdit() ? 'true' : 'false'}"><span>${typeBadge(task.issueType)}${priorityBadge(task.priority)}</span><strong>${escapeHtml(task.title)}</strong><small>${escapeHtml(task.key)} · ${escapeHtml(memberName(task.assigneeId))}</small><footer><span>${task.estimateHours ? `${task.estimateHours}h estimated` : 'No estimate'}</span><span>${escapeHtml(dual(task.dueDate))}</span></footer></button>`).join('') : '<p>No tasks</p>'}</div></section>`;
  }).join('')}</div>`;
  ...
  function taskRecord(task) {
    ...
    const linkedFiles = state.files.filter(file => file.taskId === task.id || file.projectId === task.projectId);
    const done = task.checklist.filter(item => item.done).length;
    const percent = task.checklist.length ? Math.round((done / task.checklist.length) * 100) : 0;
    const disabled = canEdit() ? '' : 'disabled';
    return `<article class="ops-record-page ops-task-record" data-record-page="task"><header class="ops-record-header"><div class="ops-record-heading"><p class="workspace-breadcrumb"><button type="button" data-ops-back-project="${escapeHtml(project?.id || '')}">${escapeHtml(project?.name || 'Tasks')}</button><span>/</span>${escapeHtml(task.key)}</p><div>${typeBadge(task.issueType)}<h1 id="modal-title" tabindex="-1">${escapeHtml(task.title)}</h1></div><p>${escapeHtml(task.key)} · Reported by ${escapeHtml(memberName(task.reporterId))}</p></div><div class="ops-record-actions"><button class="button button-secondary" type="button" data-ops-close-record>${icon('chevronLeft', 16)}Tasks</button>${canEdit() ? `<button class="button button-secondary" type="button" data-ops-edit-task="${escapeHtml(task.id)}">${icon('edit', 16)}Edit task</button><button class="button button-primary" type="button" data-ops-add-subtask="${escapeHtml(task.id)}">${icon('plus', 16)}Add subtask</button>` : ''}</div></header><div class="ops-record-body"><section class="ops-task-commandbar"><label><span>Status</span><select data-ops-task-status="${escapeHtml(task.id)}" ${disabled}>${STATUSES.map(value => `<option value="${value}" ${task.status === value ? 'selected' : ''}>${escapeHtml(statusLabel(value))}</option>`).join('')}</select></label><label><span>Assignee</span><select data-ops-task-assignee="${escapeHtml(task.id)}" ${disabled}><option value="">Unassigned</option>${state.team.filter(member => !member.pending).map(member => `<option value="${escapeHtml(member.id || member.userId)}" ${task.assigneeId === (member.id || member.userId) ? 'selected' : ''}>${escapeHtml(member.name)}</option>`).join('')}</select></label><label><span>Priority</span><select data-ops-task-priority="${escapeHtml(task.id)}" ${disabled}>${PRIORITIES.map(value => `<option value="${value}" ${task.priority === value ? 'selected' : ''}>${escapeHtml(priorityLabel(value))}</option>`).join('')}</select></label><div><span>Estimate</span><strong>${task.estimateHours || 0}h</strong></div><div><span>Logged</span><strong>${taskHours(task.id)}h</strong></div><div><span>Due</span><strong>${escapeHtml(dual(task.dueDate))}</strong></div></section><div class="ops-task-layout"><main class="ops-task-main"><section class="ops-card"><div class="ops-section-heading"><div><p class="panel-kicker">Description</p><h3>Context and implementation</h3></div></div><p class="ops-rich-copy">${escapeHtml(task.description || 'No description has been added.')}</p></section><section class="ops-card"><div class="ops-section-heading"><div><p class="panel-kicker">Definition of done</p><h3>Acceptance criteria</h3></div></div><p class="ops-rich-copy">${escapeHtml(task.acceptanceCriteria || 'No acceptance criteria have been added.')}</p></section><section class="ops-card"><div class="ops-section-heading"><div><p class="panel-kicker">Subtasks</p><h3>${children.length} linked items</h3></div>${canEdit() ? `<button class="button button-secondary button-small" type="button" data-ops-add-subtask="${escapeHtml(task.id)}">Add subtask</button>` : ''}</div>${children.length ? taskTable(sortTasks(children)) : '<div class="ops-empty"><strong>No subtasks.</strong><span>Break work down when separate ownership or estimates help.</span></div>'}</section><section class="ops-card"><div class="ops-section-heading"><div><p class="panel-kicker">Discussion</p><h3>Comments and decisions</h3></div>${canEdit() ? `<button class="button button-secondary button-small" type="button" data-ops-add-comment="${escapeHtml(task.id)}">Add comment</button>` : ''}</div>${comments(task)}</section></main><aside class="ops-task-aside"><section class="ops-card"><h3>Details</h3>${definition([['Project', `<button class="ops-link-button" type="button" data-view-project="${escapeHtml(project?.id || '')}">${escapeHtml(project?.name || 'Unassigned')}</button>`], ['Parent', task.parentTaskId ? `<button class="ops-link-button" type="button" data-ops-open-task="${escapeHtml(task.parentTaskId)}">${escapeHtml(state.tasks.find(item => item.id === task.parentTaskId)?.key || 'Task')}</button>` : 'None'], ['Reporter', escapeHtml(memberName(task.reporterId))], ['Story points', task.storyPoints || 0], ['Billable', task.billable ? 'Yes' : 'No'], ['Labels', escapeHtml(task.labels.join(', ') || 'None')]])}</section><section class="ops-card"><div class="ops-section-heading"><div><h3>Checklist</h3><p>${done}/${task.checklist.length} complete</p></div>${canEdit() ? `<button class="button button-secondary button-small" type="button" data-ops-add-checklist="${escapeHtml(task.id)}">Add</button>` : ''}</div><div class="ops-mini-progress"><span style="width:${percent}%"></span></div>${task.checklist.length ? `<div class="ops-checklist">${task.checklist.map(item => `<div><label><input type="checkbox" data-ops-toggle-checklist="${escapeHtml(task.id)}" data-check-id="${escapeHtml(item.id)}" ${item.done ? 'checked' : ''} ${disabled}><span>${escapeHtml(item.text)}</span></label>${canEdit() ? `<button type="button" data-ops-remove-checklist="${escapeHtml(task.id)}" data-check-id="${escapeHtml(item.id)}" aria-label="Remove checklist item">${icon('close', 14)}</button>` : ''}</div>`).join('')}</div>` : '<p class="ops-muted">No checklist items.</p>'}</section><section class="ops-card"><div class="ops-section-heading"><div><h3>Dependencies</h3></div>${canEdit() ? `<button class="button button-secondary button-small" type="button" data-ops-add-dependency="${escapeHtml(task.id)}">Link</button>` : ''}</div>${dependencies.length ? `<div class="ops-linked-list">${dependencies.map(item => `<div class="ops-linked-row"><button type="button" data-ops-open-task="${escapeHtml(item.id)}"><span>${statusBadge(item.status)}<strong>${escapeHtml(item.key)}</strong></span><small>${escapeHtml(item.title)}</small></button>${canEdit() ? `<button class="ops-row-remove" type="button" data-ops-remove-dependency="${escapeHtml(task.id)}" data-dependency-id="${escapeHtml(item.id)}" aria-label="Unlink dependency">${icon('close', 14)}</button>` : ''}</div>`).join('')}</div>` : '<p class="ops-muted">No blocking dependencies.</p>'}</section><section class="ops-card"><div class="ops-section-heading"><div><h3>Time</h3><p>${taskHours(task.id)}h logged</p></div>${canEdit() ? `<button class="button button-secondary button-small" type="button" data-ops-log-time="${escapeHtml(task.id)}">Log time</button>` : ''}</div>${entries.length ? `<div class="ops-time-list">${entries.slice(0, 12).map(entry => `<div><span><strong>${escapeHtml(entry.description || 'Work logged')}</strong><small>${escapeHtml(dual(entry.date))} · ${escapeHtml(memberName(entry.userId))}</small></span><span class="ops-time-value"><strong>${entry.hours}h${entry.billable ? ' · billable' : ''}</strong>${canEdit() ? `<button type="button" data-ops-remove-time="${escapeHtml(entry.id)}" data-task-id="${escapeHtml(task.id)}" aria-label="Delete time entry">${icon('trash', 14)}</button>` : ''}</span></div>`).join('')}</div>` : '<p class="ops-muted">No time logged.</p>'}</section><section class="ops-card"><h3>Files</h3>${linkedFiles.length ? `<div class="ops-linked-list">${linkedFiles.slice(0, 10).map(file => `<button type="button" data-ops-open-file="${escapeHtml(file.id)}"><span><strong>${escapeHtml(file.name)}</strong><small>${escapeHtml(file.kind || 'file')}</small></span></button>`).join('')}</div>` : '<p class="ops-muted">No project or task files.</p>'}</section></aside></div></div></article>`;
  }

  openProjectDetail = project => project && openRecord('project', project.id);
  const openTaskDetail = task => task && openRecord('task', task.id);
  const rerenderRecord = () => renderShell();

  renderTasks = function renderOperationsTasks() {
    ensure();
    const settings = state.settings.operations;
    const query = ui.query.trim().toLowerCase();
    const tasks = sortTasks(state.tasks.filter(task => matches(task, query, settings.globalTaskStatus || 'all', settings.globalTaskAssignee || 'all') && (settings.globalTaskProject === 'all' || !settings.globalTaskProject || task.projectId === settings.globalTaskProject)));
  ...
  if (previous.renderReports) renderReports = function renderConnectedReports() { return `<div class="ops-report-stack">${previous.renderReports()}${portfolioReport()}</div>`; };

  routes.projects.description = 'Manage connected delivery, work, time, files, schedule, and billing.';
  routes.tasks.description = 'Plan Jira-style work with ownership, workflow, estimates, dependencies, comments, and time.';

  window.FormcraftOpsViews = Object.freeze({ memberOptions, projectOptions, taskOptions, sortTasks, taskTable, taskBoard, projectRecord, taskRecord, openTaskDetail, rerenderRecord, portfolioReport });
  Object.assign(C, window.FormcraftOpsViews);
})();
